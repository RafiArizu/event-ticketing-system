            ...
            <section class="w-full" aria-label="Daftar vendor">
                <article class="w-full">
                    ...
                    <div class="overflow-x-auto border-y border-[#eadfd6] bg-white">
                        <table class="w-full min-w-[760px] text-left text-sm"><caption class="sr-only">Daftar pengajuan vendor</caption><thead class="border-b border-[#eadfd6] bg-[#fff0e9] text-[10px] uppercase tracking-[.14em] text-[#756861]"><tr><th scope="col" class="px-5 py-3 font-semibold">Organisasi</th><th scope="col" class="px-5 py-3 font-semibold">Kontak</th><th scope="col" class="px-5 py-3 font-semibold">Sosmed</th><th scope="col" class="px-5 py-3 font-semibold">Diajukan</th><th scope="col" class="px-5 py-3 font-semibold">Status</th><th scope="col" class="px-5 py-3 font-semibold"><span class="sr-only">Aksi</span></th></tr></thead><tbody class="divide-y divide-[#eadfd6]">
                        @forelse ($vendors as $vendor)
                            <tr class="hover:bg-[#fff9f3]"><td class="px-5 py-4"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-[5px] bg-[#f8c1ae] font-['Outfit'] font-bold text-[#762817]">{{ strtoupper(substr($vendor->organization_name, 0, 2)) }}</span><div><p class="font-semibold">{{ $vendor->organization_name }}</p><p class="mt-0.5 text-xs text-[#756861]">{{ $vendor->user?->name ?? 'Pemilik belum terhubung' }}</p></div></div></td><td class="px-5 py-4 text-[#756861]">{{ $vendor->user?->email ?? '—' }}</td><td class="px-5 py-4 text-xs leading-6 text-[#756861]"><div>Instagram: {{ $vendor->instagram ?: '-' }}</div><div>Facebook: {{ $vendor->facebook ?: '-' }}</div><div>X (Twitter): {{ $vendor->{'x_(twitter)'} ?: '-' }}</div></td><td class="px-5 py-4 text-[#756861]">{{ $vendor->created_at?->format('d M Y') ?? '—' }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] {{ $vendor->status === 'approved' ? 'bg-[#e7f3eb] text-[#287a54]' : ($vendor->status === 'rejected' ? 'bg-[#fbe8e6] text-[#b33d38]' : 'bg-[#fbf3db] text-[#956400]') }}">{{ ucfirst($vendor->status) }}</span></td><td class="px-5 py-4 text-right"><a href="{{ route('admin.vendors.show', $vendor) }}" class="font-semibold text-[#a9361f] underline decoration-[#f8c1ae] underline-offset-4">Review</a></td></tr>
                        @empty
                            <tr><td colspan="6" class="px-5 py-12 text-center text-sm text-[#756861]">Belum ada vendor yang sesuai filter.</td></tr>
                        @endforelse
                        </tbody></table>
                    </div>
                </article>
