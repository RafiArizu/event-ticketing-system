                ...
                <article class="w-full">
                    ...
                    <div class="overflow-x-auto border-y border-[#eadfd6] bg-white">
                        ...
                        @forelse ($vendors as $vendor)
                            <tr class="hover:bg-[#fff9f3]"><td class="px-5 py-4"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-[5px] bg-[#f8c1ae] font-['Outfit'] font-bold text-[#762817]">{{ strtoupper(substr($vendor->organization_name, 0, 2)) }}</span><div><p class="font-semibold">{{ $vendor->organization_name }}</p><p class="mt-0.5 text-xs text-[#756861]">{{ $vendor->user?->name ?? 'Pemilik belum terhubung' }}</p></div></div></td><td class="px-5 py-4 text-[#756861]">{{ $vendor->user?->email ?? '—' }}</td><td class="px-5 py-4 text-[#756861]">{{ $vendor->instagram ?: $vendor->facebook ?: $vendor->{'x_(twitter)'} ?: '—' }}</td><td class="px-5 py-4 text-[#756861]">{{ $vendor->created_at?->format('d M Y') ?? '—' }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em] {{ $vendor->status === 'approved' ? 'bg-[#e7f3eb] text-[#287a54]' : ($vendor->status === 'rejected' ? 'bg-[#fbe8e6] text-[#b33d38]' : 'bg-[#fbf3db] text-[#956400]') }}">{{ ucfirst($vendor->status) }}</span></td><td class="px-5 py-4 text-right"><a href="{{ route('admin.vendors.show', $vendor) }}" class="font-semibold text-[#a9361f] underline decoration-[#f8c1ae] underline-offset-4">Review</a></td></tr>
                        @empty
