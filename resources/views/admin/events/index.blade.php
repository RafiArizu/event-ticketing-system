    ...
    <main class="admin-shell-content min-w-0 lg:ml-[248px] lg:max-h-[100dvh] lg:overflow-y-auto">
        <div class="mx-auto max-w-[1400px] px-5 py-6 sm:px-7 lg:px-12 lg:py-9">
            ...
            <section aria-labelledby="event-list-title">
                <div class="mb-4 flex items-end justify-between gap-4"><div><p class="font-mono text-[10px] font-semibold uppercase tracking-[.18em] text-[#f4511e]">Daftar event</p><h2 id="event-list-title" class="mt-1 font-['Outfit'] text-2xl font-bold">Event terbaru</h2></div><span class="font-mono text-xs text-[#756861]">{{ $events->total() }} event</span></div>
                <div class="overflow-x-auto border-y border-[#eadfd6] bg-white">
                    ...
                    @forelse($events as $event)
                        <tr class="hover:bg-[#fff9f3]"><td class="px-5 py-4"><p class="font-semibold">{{ $event->title }}</p><p class="mt-0.5 text-xs text-[#756861]">{{ $event->category?->name ?? 'Tanpa kategori' }}</p></td><td class="px-5 py-4 text-[#756861]">{{ $event->vendor?->organization_name ?? '-' }}</td><td class="px-5 py-4 text-[#756861]">{{ $event->event_date?->format('d M Y') ?? '-' }}</td><td class="px-5 py-4"><span class="rounded-full {{ $event->status === 'published' ? 'bg-[#edf3ec] text-[#287a54]' : 'bg-[#fbf3db] text-[#956400]' }} px-2.5 py-1 text-[10px] font-bold uppercase tracking-[.08em]">{{ $event->status }}</span></td><td class="px-5 py-4 text-right"><a href="{{ route('admin.events.show', $event) }}" class="font-semibold text-[#a9361f] underline decoration-[#f8c1ae] underline-offset-4">Lihat detail</a></td></tr>
                    @empty
